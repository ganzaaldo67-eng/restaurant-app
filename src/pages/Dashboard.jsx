...
export default function Dashboard() {
  const [stats, setStats] = useState({ active: 0, revenue: 0, menuCount: 0 })
  ...
  async function loadData() {
    try {
      ...
      const revenue = orders
        .filter((o) => o.status === 'paid' && new Date(o.completed_at || o.created_at) >= today)
        .reduce((sum, o) => sum + Number(o.total), 0)

      setStats({ active, revenue, menuCount: menuRes.count || 0 })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="text-3xl font-bold text-green-400">RWF {stats.revenue.toLocaleString()}</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            to="/staff/take-order"
            className="px-5 py-2.5 rounded-lg bg-green-700 hover:bg-green-600 text-sm font-medium"
          >
            + New Order
          </Link>
          <Link
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/staff/active" className="text-sm text-green-400 hover:underline">
            View all →
          </Link>
        </div>
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-zinc-500 mt-0.5">
                    {new Date(o.created_at).toLocaleString()} · RWF {Number(o.total).toLocaleString()}
                  </div>
                </div>
